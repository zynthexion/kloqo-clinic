...
import type { MobileApp, User, Appointment } from "@/lib/types";
import { Eye, EyeOff, UserCircle, KeyRound, Edit, Save, X, Building } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const clinicForm = useForm<ClinicFormValues>({ resolver: zodResolver(clinicFormSchema), defaultValues: { clinicName: "" } });
  ...
  const onClinicSubmit = async (values: ClinicFormValues) => {
    ...
    try {
        await updateDoc(clinicRef, { name: values.clinicName });
        setClinicDetails(prev => prev ? {...prev, name: values.clinicName} : null);
  ...
  const handleCancelClinic = () => { if (clinicDetails) { clinicForm.reset({ clinicName: clinicDetails.name }); } setIsEditingClinic(false); }
                        ...
                        <form onSubmit={profileForm.handleSubmit(onProfileSubmit)}>
                            <CardContent className="space-y-4">
                                <FormField control={profileForm.control} name="name" render={({ field }) => (
                                    <FormItem><FormLabel>Admin Name</FormLabel><FormControl><Input {...field} disabled={!isEditingProfile} /></FormControl><FormMessage /></FormItem>
                                )}/>
                                <FormItem><FormLabel>Email Address</FormLabel><Input type="email" value={userProfile?.email || ""} disabled /></FormItem>
                                <FormField control={profileForm.control} name="phone" render={({ field }) => (
                                    <FormItem><FormLabel>Phone</FormLabel><FormControl><Input {...field} disabled={!isEditingProfile} /></FormControl><FormMessage /></FormItem>
                                )}/>
                                {isEditingProfile && (
                ...
                <Card>
                    <CardHeader>
                        <div className="flex items-center justify-between">
                             ...
                             {!isEditingClinic && (
                                <Button variant="outline" size="icon" onClick={() => setIsEditingClinic(true)}>
                                    <Edit className="w-4 h-4"/>
                                </Button>
                            )}
                        </div>
                        ...
                        <form onSubmit={clinicForm.handleSubmit(onClinicSubmit)}>
                            <CardContent className="space-y-4">
                                 <FormField control={clinicForm.control} name="clinicName" render={({ field }) => (
                                    <FormItem><FormLabel>Clinic Name</FormLabel><FormControl><Input {...field} disabled={!isEditingClinic} /></FormControl><FormMessage /></FormItem>
                                )}/>
                                <FormItem>
                                ...
                                {isEditingClinic && (
                                    <div className="flex justify-end gap-2 pt-4">
                                        <Button type="button" variant="ghost" onClick={handleCancelClinic}>Cancel</Button>
                                        <Button type="submit"><Save className="mr-2 h-4 w-4" />Save Changes</Button>
                                    </div>
                                )}
                            </CardContent>
                    ...
                    {loading ? (
                        ...
                        <form onSubmit={mobileAppForm.handleSubmit(onMobileAppSubmit)}>
                            ...
                            <FormField control={mobileAppForm.control} name="username" render={({ field }) => (
                                <FormItem><FormLabel>Username</FormLabel><FormControl><Input placeholder="mobile-user" {...field} /></FormControl><FormMessage /></FormItem>
                            )}/>
